...
const Partner = () => {
	...
	const _submitForm = async (e) => {
		...
		try {
			await axios.post('https://submit-form.com/YvF8daAg', {
				email: email,
				name: name,
				subject: subject,
				message: message,
			...
			setSubject('')
			setMessage('')
			toast('Thank you for your submission, We will be in touch!', {
				icon: '🙌',
				style: {
					borderRadius: '10px',
					background: '#333',
			...
			toast('Something wrong happened, please try again', {
				icon: '⭕',
				style: {
					borderRadius: '10px',
					background: '#ed4337',
		...
		<>
			...
			<div className="max-w-xl m-auto py-12 px-4">
				...
				<p className="text-white mt-8 text-lg">
					We love to hear about your project, it could be games, novels, comics,
					or anything else and we wanted to be part of it.
				</p>
				<p className="text-white mt-2 text-lg">
					We’re opening a partnership program where we will be working with you
					closely from the early prototype, strategy, go-to-market, funding, and
					grow it with you.
				</p>
				<form onSubmit={_submitForm}>
					...
					<div className="mt-6">
						...
						<textarea
							...
							className="resize-none h-32 w-full rounded-md p-2 outline-none"
							required
						/>
					</div>
					<div className="mt-8">
						<button
							type="submit"
							className="w-full p-2 bg-blue-800 rounded-md text-white font-semibold"
						>
							Submit
						</button>
					</div>
